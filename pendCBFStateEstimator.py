...
pi = np.pi; sin = np.sin; cos = np.cos; sinc = np.sinc; 
...
initial = np.array([0, 11.856, 0, 0, 0, 0, 0, 0])

K_gain = np.array([0.447213595500539, 3.10711058638360, 128.125101649409, 127.003953329490]) * np.array([10, 7, 1.5, 1.25])
...
h = lambda X: (1 - (X[0]/x_barr)**2)*barrSwitch[0] + (1 - (X[1]/v_barr)**2)*barrSwitch[1] + (1 - (X[2]/t_barr)**2)*barrSwitch[2] + (1 - (X[3]/w_barr)**2)*barrSwitch[3]
...
def CBFControl(X):
    ...
    qp_q = -K_desired
    qp_h = np.array([Lf + gamma*alpha(hly)]) - epsilon
    qp_G = -Lg - epsilon
...
X = integrate.odeint(func, initial, t, args = (0, ))
...
h_test = (1 - (X[:, 0]/x_barr)**2)*barrSwitch[0] + (1 - (X[:, 1]/v_barr)**2)*barrSwitch[1] + (1 - (X[:, 2]/t_barr)**2)*barrSwitch[2] + (1 - (X[:, 3]/w_barr)**2)*barrSwitch[3]
...
plt.plot(t, h_test); plt.axhline(y = 0, c = 'r'); plt.grid()
