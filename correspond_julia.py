...
def func(X, t, q):
    X_dot = np.zeros(8)
    i = int(t / Ts)
    U = CBFControl(X[:4]) # ORIGONAL 4:, use estimated
    X_dot[:4] = F(X[:4], U)
...
# X = integrate.odeint(func, initial, t, args=(1,))

X_list = np.zeros([N, 8])
for i in range(N):
    t=(i+1)/50.0
    if i==0:
        X_list[i, :] = integrate.odeint(func, initial, [0, t], args=(1,) )[1]
        print("X_RESULT: ", X_list[i, :])
    else:
        X_list[i, :] = integrate.odeint(func, X_list[i-1, :], [t-Ts, t], args=(1,) )[1]
        print("X_RESULT: ", X_list[i, :])
...
h_test = (1 - (X_list[:, 0] / x_barr) ** 2) * barrSwitch[0] + (1 - (X_list[:, 1] / v_barr) ** 2) * barrSwitch[1] + (
            1 - (X_list[:, 2] / t_barr) ** 2) * barrSwitch[2] + (1 - (X_list[:, 3] / w_barr) ** 2) * barrSwitch[3]
